...
```{r,message=FALSE}
kmeans_out<-kmeans(na.omit(base[,c('pjovem','pmotos','pop60p')]),centers = 7)
kmeans_out$size
```
...
Bom, temos que as variáveis pib percapto, quantidade de jovens matriculados no ensino médio e a densidade de veículos são estatisticamente significantes para a proxy. Tirando, claro, as variáveis que foram usadas para definir o processo gerador. 


#### Qualidade do ajuste


Temos que olhar os resíduos para verificar se nosso modelo é adequado, ou pelo menos, razoavel.
...
Ou seja, pelo p-value < 2.2e-16 rejeitamos a hipótese de que os resíduos seguem uma distribuição normal. Precisamos melhorar isso. Utilizando a função `coxbox` ela nos ajuda a obter uma transformação dos dados. Tranformação essa que pode nos ajudar a obter algumas propriedades nos resíduos. 
...
Iremos usar o varlor de x que maximiza y com a formula proposta por Box e Cox, da seguinte forma:
...
##### Tranformação da variável proxy, tranformação exp() 

```{r}
aux  = dataset_final
aux$pibpercapita = exp(-dataset_final$pibpercapita)
aux$dens_vei = exp(-dataset_final$dens_vei)
aux$pmat = exp(-dataset_final$pmat)
```
...
```{r,message = FALSE, fig.align='center'}
plot( aux$proxy_ind_acidente~dataset_final$pibpercapita,cex=1.1)
plot( aux$proxy_ind_acidente~dataset_final$dens_vei,cex=1.1)
plot( aux$proxy_ind_acidente~dataset_final$pmat,cex=1.1)
```


##### Regressão com a tranformação

```{r}
modelo <- lm(proxy_ind_acidente~pibpercapita+pmat+dens_vei,data = aux)
summary(modelo)
shapiro.test(modelo$residuals)
```
